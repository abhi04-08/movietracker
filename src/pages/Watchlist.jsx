...
const Watchlist = ({ watchlist, removeFromWatchList, toggleWatched }) => {
    return (
        <>
            <Helmet>
                <title>WatchList - CineTracker</title>
                <meta name="description" content="Add your movies to watchlist in CineTracker" />
            </Helmet>
            <div className="mb-4">
                        ...
                        {watchlist.map((movie) => (
                            <div className="relative" key={movie.imdbID}>
                                ...
                                <button onClick={() => toggleWatched(movie.imdbID)}
                                    className={`bg-green-500 absolute rounded-xl top-2 left-2 px-3 py-3 hover:scale-105${movie.watched ? "bg-green-500" : "bg-gray-600"
